...
//        [self hideLeftButtons];
        [self setSwipeOffset:0 animated:YES completion:nil];
    }
}
...
-(void) initViews
{
    ...
    panRecognizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panHandlerMGSWipe:)];
...
-(void) panHandlerMGSWipe: (UIPanGestureRecognizer *)gesture
{
    if (!_isSwipe) {
        return;
    }
