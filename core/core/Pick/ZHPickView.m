...
//@synthesize delegate;
//
//-(void)setDelegate:(void (^)(ZHPickView *, NSString *))delegate{
//    NSLog(@"ok.");
//}
...
-(instancetype)initWithPlistName:(NSString *)plistName isHaveNavControler:(BOOL)isHaveNavControler{
...
-(instancetype)initWithArray:(NSArray *)array isHaveNavControler:(BOOL)isHaveNavControler{
    self=[super init];
    if (self) {
...
-(instancetype)initWithDate:(NSDate *)defaulDate datePickerMode:(UIDatePickerMode)datePickerMode isHaveNavControler:(BOOL)isHaveNavControler{
...
//    }
    if (self.delegate) {
        self.delegate(self,_resultString);
    }
    [self removeFromSuperview];
}
/**
