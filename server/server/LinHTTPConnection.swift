...
extension HTTPConnection{
    ...
    private class var pathsAction:Dictionary<String,((connection:HTTPConnection,method:String,path:String)->protocol<NSObjectProtocol, HTTPResponse>!)>{
        ...
        dispatch_once(&YRSingleton.predicate,{
            ...
            YRSingleton.lock = NSLock();
        })
        
        return YRSingleton.instance!;
    }
    
    public class func register(method:String,path:String,action:((connection:HTTPConnection,method:String,path:String)->protocol<NSObjectProtocol, HTTPResponse>!)){
        YRSingleton.lock?.lock();
...
//            tmp.updateValue(action, forKey: "HEAD:" + path);
            tmp["GET:" + path] = action;
            tmp["POST:" + path] = action;
            tmp["HEAD:" + path] = action;
            YRSingleton.instance = tmp;

        }else{
            tmp[method.uppercaseString + ":" + path] = action;
        }
        YRSingleton.lock?.unlock();
    }
    
    public class func remove(method:String,path:String){
        YRSingleton.lock?.lock();
        var tmp = LinHTTPConnection.pathsAction;
        if(method == "*"){
            tmp.removeValueForKey("GET:" + path);
            tmp.removeValueForKey("POST:" + path);
            tmp.removeValueForKey("HEAD:" + path);
        }else{
            tmp.removeValueForKey(method.uppercaseString + ":" + path);
        }
        YRSingleton.lock?.unlock();
    }
}
...
//        var filePath = self.filePathForURI(path);
        ...
        if let action = tmp[method+":"+super.request.url().path!] {
